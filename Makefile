...
static-analysis: check-go-mod-tidy lint check-licenses validate-govulners-db-schema

.PHONY: test
test: unit integration validate-cyclonedx-schema validate-govulners-db-schema cli ## Run all tests (unit, integration, linux acceptance, and CLI tests)

.PHONY: validate-cyclonedx-schema
validate-cyclonedx-schema:
	cd schema/cyclonedx && make

.PHONY: validate-govulners-db-schema
validate-govulners-db-schema:
	# ensure the codebase is only referencing a single govulners-db schema version, multiple is not allowed
	python test/validate-govulners-db-schema.py
...
bootstrap-tools: $(TEMP_DIR)
	curl -sSfL https://raw.githubusercontent.com/nextlinux/quill/main/install.sh | sh -s -- -b $(TEMP_DIR)/ $(QUILL_VERSION)
...
unit: $(TEMP_DIR) ## Run unit tests (with coverage)
	...
	go test -coverprofile $(TEMP_DIR)/unit-coverage-details.txt $(shell go list ./... | grep -v nextlinux/govulners/test)
...
## TODO (cphillips) what does govulners have here?
...
.PHONY: validate-govulners-test-config
validate-govulners-test-config:
	# ensure the update URL is not overridden (not pointing to staging)
	@bash -c '\
		grep -q "update-url" test/govulners-test-config.yaml; \
		if [ $$? -eq 0 ]; then \
