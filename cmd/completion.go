...
var completionCmd = &cobra.Command{
	...
	Short: "Generate a shell completion for Govulners (listing local docker images)",
	Long: `To load completions (docker image list):

Bash:

$ source <(govulners completion bash)

# To load completions for each session, execute once:
Linux:
  $ govulners completion bash > /etc/bash_completion.d/govulners
MacOS:
  $ govulners completion bash > /usr/local/etc/bash_completion.d/govulners
...
# To load completions for each session, execute once:
$ govulners completion zsh > "${fpath[1]}/_govulners"

# You will need to start a new shell for this setup to take effect.

Fish:

$ govulners completion fish | source

# To load completions for each session, execute once:
$ govulners completion fish > ~/.config/fish/completions/govulners.fish
`,
