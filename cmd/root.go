...
import (
	...
	"github.com/nextlinux/govulners/internal/format"
...
var ignoreNonFixedMatches = []match.IgnoreRule{
	{FixState: string(govulnersDb.NotFixedState)},
	{FixState: string(govulnersDb.WontFixState)},
	{FixState: string(govulnersDb.UnknownFixState)},
}

var ignoreFixedMatches = []match.IgnoreRule{
	{FixState: string(govulnersDb.FixedState)},
}
...
func startWorker(userInput string, failOnSeverity *vulnerability.Severity) <-chan error {
	...
	go func() {
		...
		go func() {
			...
			str, status, dbCloser, err = govulners.LoadVulnerabilityDB(appConfig.DB.ToCuratorConfig(), appConfig.DB.AutoUpdate)
		...
		go func() {
			...
			// packages are govulners.Pacakge, not syft.Package
			// the SBOM is returned for downstream formatting concerns
			// govulners uses the SBOM in combination with syft formatters to produce cycloneDX
		...
		vulnMatcher := govulners.VulnerabilityMatcher{
			Store:          *str,
		...
		if err != nil {
			errs <- err
			if !errors.Is(err, govulnerserr.ErrAboveSeverityThreshold) {
				return
			}
		}
