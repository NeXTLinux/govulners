...
import (
	...
	"github.com/nextlinux/govulners/govulners/db/v5/store"
	...
	"github.com/nextlinux/govulners/internal/file"
	"github.com/nextlinux/govulners/internal/log"
)

const (
	FileName = govulnersDB.VulnerabilityStoreFileName
)
...
func (c *Curator) GetStore() (govulnersDB.StoreReader, govulnersDB.DBCloser, error) {
	// ensure the DB is ok
...
func (c *Curator) ImportFrom(dbArchivePath string) error {
	// note: the temp directory is persisted upon download/validation/activation failure to allow for investigation
	tempDir, err := os.MkdirTemp("", "govulners-import")
	if err != nil {
...
func (c *Curator) download(listing *ListingEntry, downloadProgress *progress.Manual) (string, error) {
	tempDir, err := os.MkdirTemp("", "govulners-scratch")
	if err != nil {
		return "", fmt.Errorf("unable to create db temp dir: %w", err)
	}
...
func (c Curator) ListingFromURL() (Listing, error) {
	tempFile, err := afero.TempFile(c.fs, "", "govulners-db-listing")
	if err != nil {
