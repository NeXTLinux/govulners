...
import (
	...
	v3 "github.com/nextlinux/govulners/govulners/db/v3"
	"github.com/nextlinux/govulners/govulners/db/v3/store/model"
)
...
func TestStore_GetID_SetID(t *testing.T) {
	dbTempFile, err := ioutil.TempFile("", "govulners-db-test-store")
	if err != nil {
...
func TestStore_MergeVulnerabilityMetadata(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			dbTempDir, err := ioutil.TempDir("", "govulners-db-test-store")
			if err != nil {
				t.Fatalf("could not create temp file: %+v", err)
			}
...
func TestCvssScoresInMetadata(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			dbTempDir, err := ioutil.TempDir("", "govulners-db-test-s")
			if err != nil {
				t.Fatalf("could not create temp file: %+v", err)
			}
			defer os.RemoveAll(dbTempDir)
