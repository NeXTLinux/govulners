...
func TestFromString(t *testing.T) {
	successTests := []struct {
	...
	}{
		{
			...
			result:          NewNamespace("alpine", govulnersDistro.Alpine, "3.15"),
		},
		{
			namespaceString: "redhat:distro:redhat:8",
			result:          NewNamespace("redhat", govulnersDistro.RedHat, "8"),
		},
		{
			namespaceString: "abc.xyz:distro:unknown:abcd~~~",
			result:          NewNamespace("abc.xyz", govulnersDistro.Type("unknown"), "abcd~~~"),
		},
		{
			namespaceString: "msrc:distro:windows:10111",
			result:          NewNamespace("msrc", govulnersDistro.Type("windows"), "10111"),
		},
		{
			namespaceString: "amazon:distro:amazonlinux:2022",
			result:          NewNamespace("amazon", govulnersDistro.AmazonLinux, "2022"),
		},
		{
			namespaceString: "amazon:distro:amazonlinux:2",
			result:          NewNamespace("amazon", govulnersDistro.AmazonLinux, "2"),
		},
		{
			namespaceString: "wolfi:distro:wolfi:rolling",
			result:          NewNamespace("wolfi", govulnersDistro.Wolfi, "rolling"),
		},
	}
