...
func (i *Index) NamespacesForDistro(d *govulnersDistro.Distro) []*distro.Namespace {
	if d == nil {
		return nil
	}
	...
	if len(versionSegments) > 0 {
		...
		if d.Type == govulnersDistro.Alpine {
		...
		// https://github.com/nextlinux/enterprise/blob/eb71bc6686b9f4c92347a4e95bec828cee879197/nextlinux_engine/services/policy_engine/__init__.py#L127-L140
		switch d.Type {
		case govulnersDistro.CentOS, govulnersDistro.RedHat, govulnersDistro.Fedora, govulnersDistro.RockyLinux, govulnersDistro.AlmaLinux, govulnersDistro.Gentoo:
			// TODO: there is no mapping of fedora version to RHEL latest version (only the name)
			distroKey = fmt.Sprintf("%s:%d", strings.ToLower(string(govulnersDistro.RedHat)), versionSegments[0])
			if v, ok := i.byDistroKey[distroKey]; ok {
				return v
			}
		}
	...
	if versionSegments == nil && d.Type == govulnersDistro.Alpine {
...
func getAlpineNamespace(i *Index, d *govulnersDistro.Distro, versionSegments []int) []*distro.Namespace {
	...
	// https://github.com/nextlinux/govulners/issues/964#issuecomment-1290888755
	distroKey := fmt.Sprintf("%s:%s", strings.ToLower(d.Type.String()), "edge")
	if v, ok := i.byDistroKey[distroKey]; ok {
		return v
	}
