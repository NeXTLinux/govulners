	...
	doc.AddRun(&sarif.Run{
		Tool: sarif.Tool{
			Driver: &sarif.ToolComponent{
				Name:           "Govulners",
				Version:        sp(v),
				InformationURI: sp("https://github.com/nextlinux/govulners"),
	...
	if pres.results.Count() > 0 {
		...
		for _, m := range pres.results.Sorted() {
			...
			out = append(out, &sarif.ReportingDescriptor{
				ID:      ruleID,
				Name:    sp(ruleName(m)),
				HelpURI: sp("https://github.com/nextlinux/govulners"),
	...
	return []*sarif.Location{
		{
			PhysicalLocation: &sarif.PhysicalLocation{
				...
				// TODO When govulners starts reporting line numbers this will need to get updated
				Region: &sarif.Region{
					StartLine:   ip(1),
					StartColumn: ip(1),
					EndLine:     ip(1),
