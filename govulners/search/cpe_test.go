...
var _ govulnersDB.VulnerabilityStoreReader = (*mockVulnStore)(nil)

type mockVulnStore struct {
	data map[string]map[string][]govulnersDB.Vulnerability
}

func (pr *mockVulnStore) GetVulnerability(namespace, id string) ([]govulnersDB.Vulnerability, error) {
	//TODO implement me
	panic("implement me")
}
...
func newMockStore() *mockVulnStore {
	pr := mockVulnStore{
		data: make(map[string]map[string][]govulnersDB.Vulnerability),
	}
	pr.stub()
	return &pr
}
...
func (pr *mockVulnStore) stub() {
	pr.data["nvd:cpe"] = map[string][]govulnersDB.Vulnerability{
		"activerecord": {
			{
...
func (pr *mockVulnStore) SearchForVulnerabilities(namespace, pkg string) ([]govulnersDB.Vulnerability, error) {
	return pr.data[namespace][pkg], nil
}

func (pr *mockVulnStore) GetAllVulnerabilities() (*[]govulnersDB.Vulnerability, error) {
	return nil, nil
}
