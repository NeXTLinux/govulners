...
import (
	...
	"github.com/nextlinux/govulners/ui"
)
...
func (h *ephemeralTerminalUI) Handle(event partybus.Event) error {
	...
	case event.Type == govulnersEvent.AppUpdateAvailable:
		if err := handleAppUpdateAvailable(ctx, h.frame, event, h.waitGroup); err != nil {
			log.Errorf("unable to show %s event: %+v", event.Type, err)
		}

	case event.Type == govulnersEvent.VulnerabilityScanningFinished:
	...
	case event.Type == govulnersEvent.NonRootCommandFinished:
		h.closeScreen(false)
